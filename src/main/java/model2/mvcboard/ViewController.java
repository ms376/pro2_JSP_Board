...
/*
 	...
 	web.xml에 기술하는 것과 동일하게 입력이 잘못되는 경우 톰캣이 시작되지 않을 수 있으므로 주의해야 한다.
 */
...
public class ViewController extends HttpServlet{
	...
	/*
		service()는 서블릿의 수명주기 메서드 중 사용자의 요청을 전송방식에 상관없이 먼저 받아 
		doGet() 혹은 doPost()로 요청을 전달하는 역할을 한다.
		따라서 전송방식에 상관없이 요청을 처리할 수 있다.
	 */
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		
		//DAO객체 생성을 통해 커넥션 풀 객체를 얻어온다.
		...
		req.getRequestDispatcher("/ms/board/dvboard.jsp").forward(req, resp);
	}
}
