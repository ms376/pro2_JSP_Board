...
/*
  글쓰기 페이지로 진입시에는 단순한 페이지 이동이므로 doGet()이 요청을 처리한다.
  즉, 글스기 페이지만 보여주면 된다.
  내용을 모두 채운 후 작성(submit)할 때는 post방식이므로 doPost()가 
  요청을 받아 처리한다.
 */
public class WriteController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		///단순한 포워드 기능만 있으면 된다.
		req.getRequestDispatcher("/ms/board/dwboard.jsp").forward(req,resp);
	}
