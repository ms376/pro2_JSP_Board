...
<script>
	...
	function deletePost() {
		...
		if (confirmed) {
			...
			form.method = "post";
			form.action = "DeleteProcess2.jsp";
...
<body>
	<jsp:include page="../board/noticeboard.jsp" />
		...
		<table border="1" width="90%">
			...
			<tr>
				...
				<td colspan="3" height="100">
					<!-- 
					입력시 줄바꿈을 위한 엔터는 \r\n으로 입력된다.
					따라서 웹브라우저에 출력시에는 <br>태그로 변경해야한다.
				 --> <!-- 엔터키로 처리된 부분을 <br>태그로 변경해야지만 줄바꿈되어 출력된다. --> <%=dto.getContent().replace("\r\n", "<br/>")%>
				</td>
			</tr>
			<tr>
				...
				/*
					...
					작성한 아이디와 일치하면 수정, 삭제버튼을 보이게 처리한다.
					즉, 작성자 본인이 해당 게시물을 조회 했을때만 수정 삭제할 수 있다.
				*/
				if(session.getAttribute("UserId")!=null
				&& session.getAttribute("UserId").toString().equals(dto.getId())){
				%>
					<button type="button" onclick="location.href='Edit2.jsp?num=<%=dto.getNum()%>';">수정하기</button>
